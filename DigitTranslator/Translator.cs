...
namespace DigitTranslator
{
    public class Translator
    {
        internal int[] Splitting(long number)//Разбиваем число на разряды 
        {
        ...
        internal string Units(int unit, int genus)//Сопоставление единиц
        {
            switch (unit)
            {
        ...
        internal string Dozens(int dozen)//Сопоставление десятков
        {
            dozen = dozen * 10;
            switch (dozen)
            {
        ...
        internal string ExceptionsDozen(int dozen)//Сопоставление чисел 10 - 19
        {
            switch (dozen)
            {
                case 10: return " десять";
        ...
        internal string Hundreds(int hundred)//Сопоставление сотен
        {
            hundred = hundred * 100;
            switch (hundred)
            {
        ...
        internal string BigDigits(int digit, int multiplier)//Вставка тысяч, миллионов, миллиардов
        {
        ...
        public string Compilation(long number)
        {
            if (number == 0) 
                return "ноль";
